...
pair<int, int> solve(int *A, int lo, int hi){
    if (hi < lo)
        return mp(INT_MIN, INT_MIN);
    if(hi == lo)
        return mp(A[lo], INT_MIN);
    int mid = (lo+hi)/2;
    pair<int, int> pl = solve(A, lo, mid), pr = solve(A, mid+1, hi), pout = mp(INT_MIN, INT_MIN);
    if (pl.ff > pr.ff) {
        pout.ff = pl.ff;
        pout.ss = max(pr.ff, pl.ss);
    }
    else {
        pout.ff = pr.ff;
        pout.ss = max(pl.ff, pr.ss);
    }
    return pout;
}
...
int main(){
    ...
    pair<int, int> p = solve(nums, 0, n-1);
    cout << p.ff << " " << p.ss << endl;
    return 0;
}