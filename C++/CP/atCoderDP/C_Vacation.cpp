...
int main(){
    ...
    int maxHappyAt[n][3];
    maxHappyAt[0][0] = arr[0][0];
    maxHappyAt[0][1] = arr[0][1];
    maxHappyAt[0][2] = arr[0][2];

    FOR (i, 1, n) {
        maxHappyAt[i][0] = max(maxHappyAt[i-1][1], maxHappyAt[i-1][2]) + arr[i][0];
        maxHappyAt[i][1] = max(maxHappyAt[i-1][0], maxHappyAt[i-1][2]) + arr[i][1];
        maxHappyAt[i][2] = max(maxHappyAt[i-1][0], maxHappyAt[i-1][1]) + arr[i][2];
    }

    int maxm = max(maxHappyAt[n-1][0], maxHappyAt[n-1][1]);
    maxm = max(maxm, maxHappyAt[n-1][2]);
    cout << maxm << endl;
