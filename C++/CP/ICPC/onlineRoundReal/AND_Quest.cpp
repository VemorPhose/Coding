...
int main(){
    ...
    while(t--){
        ...
        // FOR (i, 0, 32) {
        //     if ((k >> i) & 1) setbits[i] = 1;
        //     else setbits[i] = 0;
        // }
        // for (auto x: setbits) cout << x << " ";
        // cout << endl;
        // FOR (i, 0, n) {
        //     bool flag = true;
        //     cout << v[i] << endl;
        //     FOR (j, 0, 32) {
        //         cout << ((v[i] >> j) & 1) << " ";
        //         if ((((v[i] >> j) & 1) != 0) && setbits[j] == 1) continue;
        //         else if (((v[i] >> j) & 1) == 0 && setbits[j] == 0) continue;
        //         else {
        //             flag = false;
        //             break;
        //         }
        ...
        // }
        FOR (i, 0, n) {
            if ((k & v[i]) == k) ans.pub(i);
        }
        ull temp = ULLONG_MAX;
        for (auto x: ans) temp &= v[x];
        // cout << temp << endl;
        if (temp == k) {
            coutY;
            cout << sz(ans) << endl;
