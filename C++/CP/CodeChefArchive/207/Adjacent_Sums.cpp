...
int main(){
    ...
    while(t--){
        ...
        if(n==3) {
            cout << min(v[0]+v[2], min(abs(v[0]-v[1])+abs(v[0]-v[2]), min(abs(v[1]-v[0])+abs(v[1]-v[2]), abs(v[2]-v[0])+abs(v[2]-v[1])))) << endl;
            continue;
        }
        ll cnt1 = 0, cnt2 = 0;
        cnt1 += abs(v[1]-v[0]) + abs(v[n-1]-v[n-2]);
        cnt2 += v[0] + v[n-1];
        FOR(i, 2, n-2) {
            if(!(i&1)) {cnt1+=v[i]; cnt2+=v[i];}
        }
