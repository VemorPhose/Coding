...
int main(){
    ...
    while(t--){
        uint n; cin >> n;
        if(setbits(n)&1) {coutN; continue;}
        if(lzbits(n) < tzbits(n)) {coutN; continue;}
        string bin;
        FOR(i, 0, 64) bin.push_back('0' + ((n>>i)&1));
        for(int i = 0; i < lzbits(n)-tzbits(n); i++) bin.pop_back();
        string temp = bin; reverse(all(temp));
        if(temp == bin) coutY;
        else coutN;
    }
    return 0;
