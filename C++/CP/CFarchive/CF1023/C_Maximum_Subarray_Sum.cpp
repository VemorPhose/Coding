...
#define neginf (-1000000000000000000)

ll mss_init(vi v, string s) {
    ll cur = 0, res = 0;
    for(ll i = 0; i < v.size(); i++) {
        if(s[i] == '0') cur = max(neginf, cur+neginf);
        else cur = max(v[i], cur+v[i]);
        res = max(res, cur);
    }
    return res;
}
...
int main(){
    ...
    while(t--){
        ...
        string s; cin >> s;
        vi v(n); arrIn(v, n);
        ll temp = mss_init(v, s);

        if(k < temp) {coutN; continue;}

        else if(k == temp) {
            coutY;
            for(int i = 0; i < n; i++) {
                if(s[i] == '0') cout << neginf << " ";
                else cout << v[i] << " ";
            }
            cout << endl;
        }
