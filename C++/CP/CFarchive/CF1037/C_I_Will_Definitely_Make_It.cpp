...
int main(){
    ...
    while(t--){
        ...
        for(ll i = 0; i < n; i++) arr.pub({v[i], i});
        sort(all(arr));
        ll w = 0, flag = 1, cflag = 1;
        for(ll i = 0; i < n-1; i++) {
            if(cflag && arr[i].second != k-1) continue;
            cflag = 0;
            // cout << arr[i].first << endl;
            if(arr[i+1].first - arr[i].first > arr[i].first - w) {
                flag = 0;
                break;
            }
            w += arr[i+1].first - arr[i].first;
            // cout << arr[i].first << " " << arr[i+1].first << " " << w << endl;
        }
        if(flag) coutY;
        else coutN;
    }
