...
int main(){
    ...
    while(t--){
        ...
        while (m--) {
            string s; cin >> s;
            if (sz(s) != n) {coutN; continue;}
            int flag = 1;
            unordered_set<int> st[26];
            FOR (i, 0, n) st[s[i] - 'a'].insert(arr[i]);
            // FOR (i, 0, 26) if(!st[i].empty()) cout << *st[i].begin() << endl;
            FOR (i, 0, 26) if (sz(st[i]) > 1) flag = 0;
            FOR (i, 0, 26) {
                FOR (j, i+1, 26) {
                    if (st[i].empty()) continue;
                    else if (st[j].empty()) continue;
                    else {
                        if (*st[i].begin() == *st[j].begin()){
                            flag = 0;
                            break;
                        } 
                    }
