...
// struct comp {
//     bool operator()(const pair<ll, pair<ll, ll>>& d1, const pair<ll, pair<ll, ll>>& d2) const {
...
int main(){
    ...
    while(t--){
        ...
        vi v; vIn(v, n);
        vi h;
        FOR(0, n) h.push_back(i);
        priority_queue<pair<ll, pair<ll, ll>>, vector<pair<ll, pair<ll, ll>>>> pq;

        ll hits = 0;
        FOR(i, 0, n-1) pq.push({v[i+1]-v[i], {i, i+1}});
        while(!pq.empty()) {
            auto p = pq.top(); pq.pop();
            ll w = p.first, x = p.second.first, y = p.second.second;
        }
    }
    return 0;
}
