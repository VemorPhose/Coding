...
int n;
char prod[20][20]; // Increased size for safety
char first[20][20];
char follow[20][20];
char res[20];

// Add distinct characters to result
        ...
        for(int j = 2; prod[i][j] != '\0'; j++) {
            if(prod[i][j] == c) {
                if(prod[i][j+1] != '\0') {
                    ...
                    // If next symbol has epsilon, add Follow(LHS)
                    int hasEps = 0;
...
int main() {
    ...
    stack[top] = '$';
    stack[++top] = prod[0][0]; 
    stack[top+1] = '\0'; // Ensure string termination
    ...
    while(1) {
        ...
        // 2. Match Terminal
            ...
            printf("Match %c\n", input[ip]);
            top--; 
            stack[top+1] = '\0'; // Clean stack visual
            ip++;
        }
        // 3. Error (Terminal mismatch)
        ...
        // 4. Expand Non-Terminal
            ...
            for(int k = 0; k < n; k++) {
                if(prod[k][0] == stack[top]) {
                    ...
                    // Check Follow(LHS) if Epsilon in First
                    for(int z=0; res[z]!='\0'; z++) {
                         if(res[z] == '#') {
                             char tempFollow[20];
                             char lhs = prod[k][0];
                             // Calculate follow locally to avoid messing up loop variables
                             // (Simplified approach: assume Follow sets don't change during parse)
                             // Re-calling computeFollow is safe here because we reset 'res'
                             memset(res,0,20); 
                             computeFollow(lhs);
            ...
            if(found != -1) {
                ...
                stack[top+1] = '\0'; // Ensure termination
            } else {
